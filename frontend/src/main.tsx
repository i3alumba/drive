...
function viewUrl(path: string) {
  return `/api/view?path=${encodeURIComponent(path)}`;
}
function previewUrl(path: string) {
  return `/api/preview?path=${encodeURIComponent(path)}`;
}
function subtitleUrl(path: string) {
  return `/api/view?path=${encodeURIComponent(path.replace(/\.[^/.]+$/, ".vtt"))}`;
}
